...
A workflow is a set of automated steps that Subflow runs on behalf of your care team when a specific event occurs. Instead of staff manually sending a follow-up message after every form submission or creating a task each time a patient reaches a certain point in their care journey, workflows handle those actions automatically. The result is that the right message, task, or update happens at the right time, without requiring anyone to remember to do it.

## What a workflow can do

Each step in a workflow is called a workflow node, and nodes can do different things depending on what the workflow needs.
